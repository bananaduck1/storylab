  ...
  useEffect(() => {
    if (active) {
      ...
      intervalRef.current = setInterval(() => {
        // Fast to ~60%, slows down, then trickles to 100%
        if (p < 60) p += 3 + Math.random() * 2;
        else if (p < 80) p += 0.8 + Math.random() * 0.5;
        else if (p < 95) p += 0.3 + Math.random() * 0.2;
        ...
        <div className="mt-4 space-y-4">
          {analysis.rubric_scores.map((rs) => (
            <div key={rs.rubric_id}>
              ...
              {rs.anchor_quote && (
                <blockquote className="mt-2 border-l-2 border-zinc-200 pl-3 text-xs leading-relaxed">
                  <p className="italic text-zinc-700">&ldquo;{rs.anchor_quote}&rdquo;</p>
                </blockquote>
              )}
...
export default function AiEditorPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-16">
      ...
      <form onSubmit={isPro ? handleProStart : handleReportSubmit} className="mt-8 space-y-5">
        <div>
          <label className="block text-sm font-medium text-zinc-900 mb-1">
            Essay file <span className="font-normal text-zinc-500">(txt, pdf, or docx — max 10 MB)</span>
          </label>
          <div
            onDrop={handleDrop}
          ...
          >
            ...
            <input ref={fileRef} type="file" accept={ACCEPT} className="hidden" onChange={(e) => handleFile(e.target.files?.[0] ?? null)} />
          </div>
          {file && (
            <button type="button" onClick={() => { setFile(null); if (fileRef.current) fileRef.current.value = ""; }} className="mt-2 text-xs text-zinc-500 underline hover:text-zinc-700">
              Remove file
            </button>
          )}
        </div>
        ...
        {!chatStarted && (
          <div>
            <label htmlFor="prompt" className="block text-sm font-medium text-zinc-900">
              {isPro ? "First message to your coach" : "Prompt"} <span className="font-normal text-zinc-500">(optional)</span>
            </label>
            <textarea
              id="prompt"
              rows={2}
              ...
              placeholder={isPro ? "e.g. I'm stuck on the ending — can you help?" : "e.g. Focus on narrative structure and voice."}
            />
          </div>
        )}

        {!chatStarted && (
          <button type="submit" disabled={loading || !file} className="inline-flex items-center justify-center rounded-full bg-zinc-900 px-6 py-3 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 disabled:opacity-50">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-medium text-zinc-600">
              {progress < 30 ? "Reading your essay…" : progress < 65 ? "Coaching in progress…" : progress < 95 ? "Finalizing feedback…" : "Done!"}
            </span>
            <span className="text-xs tabular-nums text-zinc-500">{Math.round(progress)}%</span>
          </div>
          <div className="h-1.5 w-full rounded-full bg-zinc-100 overflow-hidden">
            <div className="h-1.5 rounded-full bg-zinc-900 transition-all duration-300 ease-out" style={{ width: `${progress}%` }} />
          </div>
        </div>
      )}
      ...
      {isPro && chatStarted && (
        <ProChatView messages={chatMessages} loading={loading} onSend={handleProSend} suggestedActions={chatSuggestedActions} />
      )}

      {!isPro && reportResult && (
        <AnalysisReport data={reportResult} tier={tier} onUpgrade={() => { setTier("pro"); resetResults(); }} />
      )}
    </div>
  );
}
