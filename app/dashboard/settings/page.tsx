...
export default async function SettingsPage() {
  ...
  const { data: teacher } = await getSupabase()
    .from("teachers")
    .select("id, name, email, subject, agent_config, bio, photo_url, quote, pricing_config, ai_coaching_enabled, live_sessions_enabled, primary_emphasis, storefront_content, storefront_published")
    .eq("user_id", user.id)
    .maybeSingle();
