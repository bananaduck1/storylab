...
export default async function ConfirmedPage({ searchParams }: PageProps) {
  ...
  if (booking_id) {
    ...
    if (data) {
      // Supabase returns availability as an array when using foreign key select
  ...
  // Fallback if booking not found (e.g., direct nav without ID)
  if (!booking) {
    return (
      <div className="mx-auto w-full max-w-2xl px-6 py-24 text-center">
        ...
        <p className="mt-4 text-base leading-relaxed text-zinc-500">
          Thank you for booking a Parent Consultation. You&rsquo;ll receive a confirmation email
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-24">
      ...
      <p className="mt-5 text-lg leading-relaxed text-zinc-500">
        Hi {booking.parent_name} — your consultation is confirmed. A confirmation email is on its
        way to <strong className="font-medium text-zinc-700">{booking.parent_email}</strong>.
      </p>
        ...
        <dl className="mt-5 space-y-4">
          <div>
            ...
            <dd className="mt-0.5 text-base font-semibold text-zinc-900">
              Parent Consultation &middot; 1 hour
            </dd>
          </div>

          {slotTime && (
            <div>
              <dt className="text-sm font-medium text-zinc-400">Date &amp; Time</dt>
          ...
          <div>
            <dt className="text-sm font-medium text-zinc-400">Zoom Link</dt>
            <dd className="mt-0.5 text-base text-zinc-600">
              Sam will send you a Zoom link 24 hours before your session.
            </dd>
          </div>
        </dl>
      </div>
      ...
      <div className="mt-8 rounded-2xl border border-zinc-100 bg-white/60 p-8">
        ...
        <ul className="mt-4 space-y-3 text-base leading-relaxed text-zinc-600">
          ...
          <li className="flex items-start gap-3">
            <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-zinc-300" />
            Write down your top 2–3 questions about the application process.
          </li>
          <li className="flex items-start gap-3">
            <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-zinc-300" />
            No formal preparation required — the more context you can share, the more useful our
