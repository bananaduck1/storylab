...
function makeSteps(teacherName: string) {
  return [
    { title: `What should ${teacherName} call you?`, subtitle: "Let's get acquainted before we dive into your essays." },
    { title: "What schools are you considering?", subtitle: "No pressure — you can always update this later." },
    { title: `Tell ${teacherName} about your essays`, subtitle: `This helps ${teacherName} understand where you are in the process.` },
    { title: "What do you most want to get out of this?", subtitle: `The more ${teacherName} knows, the better the coaching.` },
  ];
}

export default function OnboardingClient({ teacherName }: { teacherName: string }) {
  const router = useRouter();
  const STEPS = makeSteps(teacherName);
